...
import com.itis.bugtracker.BugTrackerLibApi.repositories.TaskRepository;
...
public class TaskServiceImpl implements TaskService {
    ...
    public Task changeState(Task task, User user, Task.Status status) {
        ...
        if (checkConstraints(task, user)) {
            task.setStatus(task.nextStatus());
            task.setExecutor(user);
    ...
    @Override
    public boolean checkConstraints(Task task, User executor) {

        //executor == null -> task has no executor
        Task.Status nextStatus = task.nextStatus();

        if (executor == null) {
            return !nextStatus.equals(Task.Status.IN_PROGRESS);
        }
        ...
        if ((nextStatus.equals(Task.Status.IN_PROGRESS)
                || nextStatus.equals(Task.Status.DEV_TEST)
                || nextStatus.equals(Task.Status.REVIEW))
                && role.equals(User.Role.TEST_ENGINEER)) {
            return false;
        }
        return !nextStatus.equals(Task.Status.TESTING) || !role.equals(User.Role.DEVELOPER);
    }
